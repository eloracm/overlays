...
GPXParser.prototype.parse = function (gpxstring) {
    ...
    for (let idx in trks) {
        ...
        for (let idxIn in trkpts) {
            ...
            // --- Parse extensions (HR + Speed) ---
            const ext = trkpt.querySelector("extensions");
            if (ext) {
                const tpx = ext.querySelector("gpxtpx\\:TrackPointExtension, TrackPointExtension");
                if (tpx) {
                    const hr = tpx.querySelector("gpxtpx\\:hr, hr");
                    const speed = tpx.querySelector("gpxtpx\\:speed, speed");

                    if (hr && hr.textContent)
                        pt.heartRateBpm = parseFloat(hr.textContent);

                    if (speed && speed.textContent)
                        pt.speed = parseFloat(speed.textContent); // meters per second
                }
            }

            // --- Inherit HR/Speed if missing ---
            if (trackpoints.length > 0) {
                const prev = trackpoints[trackpoints.length - 1];
                if (pt.heartRateBpm == null && prev.heartRateBpm != null)
                    pt.heartRateBpm = prev.heartRateBpm;
                if (pt.speed == null && prev.speed != null)
                    pt.speed = prev.speed;
            }

            // --- Convert to mph if available ---
            if (pt.speed != null)
                pt.speedMph = pt.speed * 2.236936; // 1 m/s = 2.236936 mph
