...
// -------------------- DRAW HEART RATE OVERLAY (SYNCED TO ACTUAL BPM, RIGHT EDGE) --------------------
function drawHeartRateOverlay(currentTime) {
    if (!points || points.length < 2) return;

    // Find the closest trackpoint in time
    const targetTime = gpxStartMs + (currentTime / video.duration) * gpxDurationMs;
    let closest = points[0];
    let minDiff = Math.abs(points[0].time.getTime() - targetTime);
    ...
    const hr = Math.max(40, Math.min(220, closest.hr)); // clamp realistic HR range

    // Beat frequency (BPM → beats per second)
    const beatsPerSecond = hr / 60;
    const beatPeriodMs = 1000 / beatsPerSecond;
    const t = Date.now() % beatPeriodMs;
    const beatPhase = (t / beatPeriodMs) * 2 * Math.PI;

    // Pulse scaling — base pulse from beat phase + slight HR-based size factor
    const pulse = 1 + 0.15 * Math.sin(beatPhase);
    const hrScale = 1 + (hr - 100) / 800; // larger at high HR

    // Heart position
    const cx = canvas.width - 100;  // right side
    const cy = canvas.height / 2;   // vertically centered
    const baseSize = 25;
    const size = baseSize * pulse * hrScale;
    ...
    ctx.beginPath();
    ctx.moveTo(0, -10);
    ctx.bezierCurveTo(10, -20, 25, -10, 0, 20);
    ...
    ctx.fillText(`${Math.round(hr)} BPM`, cx, cy + 40);
}
...
function drawOverlay(currentTime) {
    ...
    // --- Draw overlays ---
    drawHeartRateOverlay(currentTime);
...
async function exportFrames() {
    const frameCanvas = document.getElementById("overlay");
    if (!frameCanvas) {
    ...
    for (let f = 0; f < totalFrames; f++) {
        ...
        // Draw your overlays at this frame
        drawOverlay(time);
        ...
        if (!blob) {
            console.warn(`Frame ${f} skipped — no blob created`);
            continue;
        }

        const buf = await blob.arrayBuffer();

        await fetch(`http://localhost:8080/save-frame?f=${String(f).padStart(5, "0")}`, {
            method: "POST",
            headers: {
                "Content-Type": "image/png", // ✅ This is critical!
            },
            body: buf,
        });
    ...
    console.log("✅ All frames exported — ready for ffmpeg merge!");
}
